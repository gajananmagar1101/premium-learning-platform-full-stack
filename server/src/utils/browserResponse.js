        ...
        <div class="card-grid">
          <div class="content">
            ...
            <div class="stack">
              ...
              ${hint ? `<div class="note">${escapeHtml(hint)}</div>` : ''}
              <div class="actions">
                ${appUrl ? `<a class="button button-primary" href="${escapeHtml(appUrl)}">Open App</a>` : `<a class="button button-primary" href="/">Open API Home</a>`}
                <a class="button button-secondary" href="/">API Home</a>
          ...
          <aside class="aside">
            ...
            <div class="info-list">
              <div class="info-card">
                ...
                <span>The server is online and responding correctly from the deployed environment.</span>
              </div>
              <div class="info-card">
                <strong>Action is required</strong>
                <span>This screen usually means the request needs a fresh login, a valid token, or access through the frontend app.</span>
              </div>
              <div class="info-card">
                <strong>Best next step</strong>
                <span>${appUrl ? 'Open the frontend app and continue from there for the full product experience.' : 'Return to the app entry point and continue from the login flow.'}</span>
              </div>
            </div>
            ${statusCode ? `<div class="status-code">HTTP ${escapeHtml(statusCode)}</div>` : ''}
          </aside>
        </div>
      </section>
    </div>
...
const sendBrowserMessagePage = (req, res, options) => {
  if (!wantsHtml(req)) return false
  res.status(options.statusCode || 200).type('html').send(buildPage(options))
  return true
}

module.exports = { wantsHtml, sendBrowserMessagePage }

...
type SubjectApiResponse = { data: { subjects?: unknown[] } }
const subjectsByYearCache = new Map<string, Promise<SubjectApiResponse>>()
...
export const subjectAPI = {
  ...
  getByYearCached: (yearId: string) => {
    const key = yearId.trim().toUpperCase()
    const cached = subjectsByYearCache.get(key)
    if (cached) {
      return cached
    }

    const request = api.get(`/subjects/${key}`)
      .finally(() => {
        window.setTimeout(() => {
          subjectsByYearCache.delete(key)
        }, 5 * 60 * 1000)
      })
    subjectsByYearCache.set(key, request)
    return request
  },
  create: (data: { name: string; yearId: string }) => api.post('/subjects', data),
  delete: (id: string) => api.delete(`/subjects/${id}`),
  clearYearCache: (yearId?: string) => {
    if (!yearId) {
      subjectsByYearCache.clear()
      return
    }
    subjectsByYearCache.delete(yearId.trim().toUpperCase())
  },
}

...
export function Subjects() {
  ...
  const handleDeleteSubject = async (subject: SubjectOption) => {
    const yearName = years.find((year) => year.id === subject.yearId)?.name ?? 'this year'
    if (!(await confirm({
      title: 'Remove Subject',
      message: (
        <>
          Are you sure you want to remove <span className="font-semibold">{subject.name}</span> from{' '}
          <span className="font-semibold">{yearName}</span>?<br/><br/>
